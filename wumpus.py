...
class WumpusEnvironment(XYEnvironment):
    ...
    def percept(self, agent):
        ...
        scream = None
        if self._do_scream:
            WumpusEnvironment.scream_num = WumpusEnvironment.scream_num + 1
            scream = 'Scream'
            self._do_scream = False
        return (stench, breeze, glitter, bump, scream)

    def execute_action(self, agent, action, seed):
            ...
            if self[agent]==self.START_SQUARE:
                if any([ isinstance(i,Gold) for i in agent._holding ]):
                    ...
                    logging.critical('You win!!! Total score: {}'.format(
                        agent.performance))
                    print("yay")
                    WumpusEnvironment.win_move = WumpusEnvironment.win_move + 1
                    print(WumpusEnvironment.win_move)
                    print("Scream")
                    print(WumpusEnvironment.scream_num)
                else:
                    if agent.performance == -1:
                        print("just left move")
                        WumpusEnvironment.just_left_num = WumpusEnvironment.just_left_num + 1
                        print(WumpusEnvironment.just_left_num)
                    else:
                        # quit()
                        print("more move")
                        if agent.performance <-40:
                            WumpusEnvironment.moved_alot[seed] = agent.performance
                        WumpusEnvironment.more_move_num = WumpusEnvironment.more_move_num + 1
                        print(WumpusEnvironment.more_move_num)
    ...
    def add_walls(self, wall_prob=.05):
        for x in range(self.width):
            for y in range(self.height):
                if ((self.CAN_COEXIST or (x,y) not in self.values()) and
                        (x,y) != self.START_SQUARE and 
